...
cap1 = cv2.VideoCapture('rtsp://admin:admin123@192.168.199.67:554/cam/realmonitor?channel=2&subtype=0&unicast=true&proto=Onvif')
            ...
            if len(out1[0])>0:#,len(out2[0])
                if (out1[0][1])>0.96:#,(out2[0][1])
                    if (len(out1[0][0]))==7 or 8:#,(len(out2[0][0]))
                        ...
                        data = 'id2',out1,localtime
                        #data = str('id'+rtsp[65]+str(out)+str(localtime))
                        #print(data)
                        data2 = str(data)
                        s.sendto(data2.encode(),('192.168.199.61',1717))#78,1712
                        print(data2)
                        #print(s.recv(1024))
                        s.close
    c = c+1
